...
export class CurrencyRateStore {
    ...
    constructor() {
        ...
        this.changeHandlerCurrencyRadio = this.changeHandlerCurrencyRadio.bind(this);
    }
    ...
    public changeHandlerCurrencyRadio(e: React.ChangeEvent<HTMLInputElement>) {
        runInAction(() => {
            this.currencyValue = e.target.value;
        });
        this.convertCurrency();
    ...
    public getCurrencyDay(date: string) {
        Promise.all([
            server.getCurrencyList(date, CurrencyCode.USD)
                .then((currency) =>
                    runInAction(() => {
                        this.amountUSD = currency.Cur_OfficialRate;
                    })),
            server.getCurrencyList(date, CurrencyCode.EUR)
                .then((currency) =>
                    runInAction(() => {
                        this.amountEUR = currency.Cur_OfficialRate
                    })),
            server.getCurrencyList(date, CurrencyCode.RUB)
                .then((currency) =>
                    runInAction(() => {
                        this.amountRUB = currency.Cur_OfficialRate
                    })),
            server.getCurrencyList(date, CurrencyCode.PLN)
                .then((currency) =>
                    runInAction(() => {
